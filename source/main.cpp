...
// Just in here while I'm experimenting, will move to another file
...
std::vector<ImageDocument*> documents;
...
int main(int, char**)
{
    ...
    while (!done)
    {
			  ...
			  if (ImGuiFileDialog::Instance()->IsOk == true)
			  {
				  ...
				  for (std::map<std::string, std::string>::iterator it = selection.begin(); it != selection.end(); it++)
				  {
					  ...
					  if (image)
					  {
						  ...
						  documents.push_back(new ImageDocument(it->first, it->second, image));
					  }
					  else
					  {
						  LOG("Failed %s\n", it->second.c_str());
		...
		{
			documents[ idx ]->Render();

			// Delete the document if the user closes it
			if (documents[ idx ]->IsClosed())
			{
				delete documents[ idx ];
				documents.erase(documents.begin() + idx);
